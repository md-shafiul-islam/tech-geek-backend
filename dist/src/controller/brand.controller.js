...
class BrandController {
  async getByBrandName(req, resp) {
    try {
      ...
      if (brand) {
        ...
        resp.status(202);
        resp.send(
          (0, respFormat_1.default)(brand, "Brand not Found by given id", true)
        );
      }
    } catch (error) {}
  }
  async getAllProductsByBrand(req, resp) {
    ...
    const products = await product_service_1.productService.getAllProductBrand(
      req.query
    );
    if (products) {
      resp.status(200);
      resp.send(
        (0, respFormat_1.default)(
          products,
  ...
  async delete(req, resp) {
    ...
    if (intId > 0) {
      ...
      if (deleteResp) {
        resp.status(202);
        resp.send(
          (0, respFormat_1.default)(deleteResp, "Brand deleted ", true)
        );
      }
    }
    resp.status(202);
